...
FT.Interactive = FT.Interactive || {
    apps:[],
    setRef: function(uuid){
        var eList = document.getElementsByTagName('div');
        for (var i = 0; i < eList.length; i++) {
            if (eList[i].getAttribute('id') === 'ig'+uuid) {
                FT.Interactive.apps.push({
                    self: eList[i],
					uuid: uuid
                })
                return (FT.Interactive.apps.length - 1);
            }
        }
    },
	/*
		checks for presence of base css file if not present add
	*/
    setStyle: function(){
        var cssList = document.getElementsByTagName('link');
        var flag = false;
        for (var i = 0; i < cssList.length; i++) {
            if (cssList[i].getAttribute('rel') === "stylesheet" && cssList[i].getAttribute('type') === "text/css" && cssList[i].getAttribute('href') === 'http://interactive.ftdata.co.uk/css/FTi-base.css') {
                flag = true;
                break;
            }
        }
        if (!flag) {
            var cLnk = document.createElement("link");
            cLnk.setAttribute("rel", "stylesheet");
            cLnk.setAttribute("type", "text/css");
            cLnk.setAttribute("href", 'http://interactive.ftdata.co.uk/css/FTi-base.css');
            document.getElementsByTagName("head")[0].appendChild(cLnk);
        }
    },
	/*
		buildSpace gets called with the conf.dimensions and the id of the interactive parameter
	*/
	buildSpace: function(FTi){
		$(FTi.self).html('<div id="FTi"><div></div></div>');
		...
		switch(FTi.conf.dimensions[0]){
			...
			case 566:
				...
				break;
			default:
			$(FTi.self).find('div#FTi div').css({'width':FTi.conf.dimensions[0]+'px','height': FTi.conf.dimensions[1]+'px'});
				break;
		}
	...
	*/
	convertUuid: function(p){
		var url = 'http://interactive.ftdata.co.uk/features/2012-10-22_appleTimeline/';
		return url;
	},
	/*
		error messsage output with a text string output
	*/
	failOutput: function(msg){}
};
